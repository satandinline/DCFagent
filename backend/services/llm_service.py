...
class LLMService:
    ...
    async def extract_financial_data_optimized(
        self, 
        text: str, 
        prompt_type: str = 'financial_extraction'
    ) -> dict[str, Any]:
        """Extract financial data using optimized prompt from 15-iteration optimization
        
        Args:
            text: Financial report text
            prompt_type: Type of prompt template (see prompts.py for available templates)
        ...
        try:
            ...
            logger.warning(f"Prompt '{prompt_type}' not found, using default")
            system_prompt = get_prompt('financial_extraction')
        ...
        try:
            ...
            raw = response.choices[0].message.content or ""
            logger.info("Optimized LLM response length: %d chars", len(raw))
            
            # Try to extract JSON from response
            data = self._extract_json(raw)
            logger.info("Optimized extraction succeeded")
            return data
            
        except json.JSONDecodeError as exc:
            logger.error("Failed to parse optimized LLM JSON: %s", exc)
            # Fallback to original method
            logger.warning("Falling back to original extraction method")
            return await self.extract_financial_data(text)
        except Exception as exc:
            logger.error("Optimized extraction failed: %s", exc)
            raise
